...
function exportToExcel(items,counts){
  const rows=[...items].sort((a,b)=>a.storeOrder-b.storeOrder).map(item=>({
    ...
    "Count Unit":item.countUnit||item.unit,"Order Unit":item.orderUnit||item.unit,
      ...
      if (d?.items && d.items.length > 0) {
        ...
        const merged = SOURCE_ITEMS.map(item => {
          ...
          return {
            ...
            countUnit: saved.countUnit ?? item.countUnit,
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8,paddingBottom:20}}>
            {filteredManage.map(item=>(
              <div key={item.id} style={{...s.mCard,opacity:item.active==="No"?0.6:1}}>
                ...
                <div style={s.mFields}>
                  {[["location","Location",LOCATIONS,140],["category","Category",CATEGORIES,120],["unit","Unit (default)",UNITS,90]].map(([field,label,opts,w])=>(
                  ...
                  {[
                    ...
                    ["countUnit","Count Unit","text",80],
                    ["countPerOrderUnit","Per Order Unit","number",60],
                    ["orderUnit","Order Unit","text",80],
